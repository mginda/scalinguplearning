...
#               - Ginda, M., Richey, M. C., Cousino, M., & B?rner, K. (2019). 
...
# Expression for fig 1 axis labels
a_ylab <- expression(paste("Dwell time (",italic(h),")"))
b_xlab <- expression(paste("Learning objectives (",italic("target nodes"),")"))

# Update learning objective level label for weeks 4-5
levels(act_s$learningObjective)[4] <- "LO 4-5"
...
a <- ggplot(eds, aes(x=date, y=dwellTime, fill=learningObjective)) + 
            ...
            theme(plot.tag = element_text(face='bold'),
                  axis.title.x = element_text(hjust=1, colour="grey20",
...
g.mid <- ggplot(act_s,aes(x=1,y=learningObjective))+
                ...
                labs(y=NULL)+
                scale_x_continuous(expand=c(0,0),limits=c(0.92,1.09))+
...
b <- ggplot(edges_s,aes(x=to,y=from)) + 
            ...
            theme(plot.tag = element_text(face='bold'),
                  axis.title.x = element_text(hjust=.065, colour="grey20",
                                              margin=unit(c(3,0,0,0),"mm")),
                  axis.ticks.x.bottom = element_line(colour="grey60", linetype = "solid"),
                  axis.line.x = element_blank(),
...
png(filename=paste0(path_output,"/results/visualization/panel-area+heatmap-divg-logNormed-labsAvgTrans.png"),
    ...
    type="cairo", bg = "white")
grid.arrange(a,g.mid,b,ncol=3,widths=c(3.70/10,.55/10,4.75/10))
dev.off()
